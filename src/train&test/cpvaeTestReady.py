...
if weighs is not None:
    if weighs[0] != -1:
        slowdos_weights[0] = weighs[0]

    if weighs[1] != -1:
        slowdos_weights[1] = weighs[1]
...
accuracy, precision, recall, f1, auc, cr = CPVAE_model.evaluate(slowdos_train_loader, CPVAE_criterion, evaluation_on="train")
